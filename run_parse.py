...
for file_name in glob.glob("html_files/*.html"):
	#file_name = "html_files/20230928120319.html"

	file = open(file_name, "r")
	soup = BeautifulSoup(file.read(), "html.parser")
	file.close()

	#find the metatagname that you want from html, note it is case sensitive
	...
	#want the top 3 accounts that consume gas
	...
	#use the file name to get the date time scraped
	...
	#scrape_time_hour = scrape_time[9:10]
	#scrape_time_minute = scrape_time[11:12]
...
dataset.to_csv("parsed_files/datset.csv", index=False)
